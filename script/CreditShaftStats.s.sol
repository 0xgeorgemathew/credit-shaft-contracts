...
contract CreditShaftStats is Script {
    ...
    function _displayActivePositionsSummary() internal view {
        ...
        for (uint256 i = 0; i < 100; i++) {
            // Reasonable limit to prevent gas issues
            try leverageContract.activeUsers(i) returns (address user) {
                (uint256 collateralLINK,, uint256 borrowedUSDC, uint256 suppliedLINK,,,,, bool isActive,,,,) =
        ...
        if (totalCollateralLINK > 0) {
            ...
            uint256 totalCollateralUSD =
                (totalCollateralLINK * linkPrice) / (10 ** (LINK_DECIMALS + PRICE_FEED_DECIMALS - USDC_DECIMALS));
            uint256 totalExposureUSD =
                (totalSuppliedLINK * linkPrice) / (10 ** (LINK_DECIMALS + PRICE_FEED_DECIMALS - USDC_DECIMALS));
        ...
        if (validLTVCount > 0) {
            ...
            console.log(
                "  - Average LTV:               %s %s%% (%s)", avgLTVIcon, _formatAmount(avgLTV, 2, 2), avgLTVStatus
            );
        }
        console.log(unicode"  ╰─────────────────────────────────────────╯");
    }
            ...
            try leverageContract.activeUsers(i) returns (address user) {
                ...
                if (isActive) {
                    ...
                    if (entryPrice > 0) {
                        ...
                        uint256 entryValue =
                            (suppliedLINK * entryPrice) / (10 ** (LINK_DECIMALS + PRICE_FEED_DECIMALS - USDC_DECIMALS));
                    ...
                    console.log(
                        "  Current LTV:         %s %s%% (%s)", ltvIcon, _formatAmount(currentLTV, 2, 2), ltvStatus
                    );
    ...
    function _calculateLTV(address user) internal view returns (uint256) {
        (,, uint256 borrowedUSDC, uint256 suppliedLINK,,,,, bool isActive,,,,) = leverageContract.positions(user);
    ...
    function _formatAddress(address addr) internal pure returns (string memory) {
        ...
        for (uint256 i = addrBytes.length - 4; i < addrBytes.length; i++) {
